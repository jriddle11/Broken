...
namespace Broken.Common.ParticleSystem
{
    public class ParticleSystem : IGameObject
    {
        ...
        public ParticleSystem(Vector2 position, string texture, Color color, float opacity = 1f, float speed = 80f, double pace = 0.2, float lifeTime = 5f, int maxParticles = 10)
        {
            Position = position;
        ...
        private void GenerateNewParticle()
        {
            ...
            float scale = (float)(_random.NextDouble() * 0.5 + 0.8);
        ...
        public void Draw(SpriteBatch spriteBatch)
        {
            foreach (var particle in _particles)
            {
                ...
                Color color = particle.Color * remainingLife * _opacity;
