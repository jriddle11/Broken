...
namespace Broken.Scripts.MainGame
{
    public abstract class Entity : IGameObject
    {
        ...
        {
            return GameController.Instance.GetRoomDepth(FloorCollider);
        }

        protected virtual void CheckRoomCollisions()
        {
            var rects = GameController.Instance.CurrentRoom.RectangleColliders;
            if (rects != null)
            {
        ...
        {
            /* DISABLED
            foreach (var entity in GameController.Instance.CurrentEntities)
            {
